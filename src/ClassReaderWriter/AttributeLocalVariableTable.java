...
public class AttributeLocalVariableTable extends AttributeInfo {
    ...
    AttributeLocalVariableTable(final u2 nameIndex, final String type, final PosDataInputStream posDataInputStream)
      ...
      public void WriteAttribute(DataOutputStream dos)
       throws IOException, Exception {
        dos.writeShort((short)this.local_variable_table_length.value);
        
        if (this.local_variable_table_length.value > 0) {
           
            for (int i = 0; i < this.local_variable_table_length.value; i++) {
                this.localVariableTable[i].WriteLocalVariableTable(dos);
            }
        }
    ...
    public final class LocalVariableTable extends ClassComponent {
       ...
       public void WriteLocalVariableTable(DataOutputStream dos)
       throws IOException, Exception {
            dos.writeShort((short)this.start_pc.value);
            dos.writeShort((short)this.length_lvt.value);
            dos.writeShort((short)this.name_index.value);
            dos.writeShort((short)this.descriptor_index.value);
            dos.writeShort((short)this.index.value);
        }
        /**
