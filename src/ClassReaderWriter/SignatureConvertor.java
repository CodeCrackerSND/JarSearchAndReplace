...
final class SignatureConvertor {
    ...
    private static String rawSignature2Type(final String signature)
        ...
        switch (signatureLength) {
            case 0:
            case 2:
                throw new Exception(String.format("Sinagure length cannot be 0, or 2. Current 'signature' length=%d.", signatureLength));
    ...
    private static String extractClassFullSignature(final String classSignature)
        ...
        if (length < 3) {
            throw new Exception(String.format("Fully-qualified class sinagure length cannot be less than 2. Current length=%d.", length));
        }
        if ((classSignature.charAt(0) != 'L') || (classSignature.charAt(length - 1) != ';')) {
            throw new Exception(String.format("Fully-qualified class sinagure format is not 'L-xxx-;'. it is '%s'.", classSignature));
        }
