...
public class AttributeCode extends AttributeInfo {
    ...
    AttributeCode(final u2 nameIndex, final String type, final PosDataInputStream posDataInputStream, final AbstractCPInfo[] cp)
       ...
       throws IOException, Exception {
        dos.writeShort((short)this.max_stack.value);
        dos.writeShort((short)this.max_locals.value);
        dos.writeInt((int)this.code.length);
        dos.write(this.code, 0, this.code.length);
        dos.writeShort((short)this.exception_table_length.value);
        int i=0;
        ...
        if (this.exception_table_length.value > 0)
        {
            ...
            {
            this.exceptionTable[i].WriteExceptionTable(dos);
            }
        }

        dos.writeShort((short)this.attributes_count.value);
        if (this.attributes_count.value > 0) {
            
            for (i = 0; i < this.attributes_count.value; i++) {
                this.attributes[i].WriteAttrib(dos);
            }
    ...
    public final class ExceptionTable extends ClassComponent {
        ...
        private ExceptionTable(final PosDataInputStream posDataInputStream)
          ...
          public void WriteExceptionTable(DataOutputStream dos)
       throws IOException, Exception {
            dos.writeShort((short)this.start_pc.value);
            dos.writeShort((short)this.end_pc.value);
            dos.writeShort((short)this.handler_pc.value);
            dos.writeShort((short)this.catch_type.value);
        }
        /**
